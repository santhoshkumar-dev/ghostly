...
export const Home: React.FC = () => {
  ...
  const runAIStream = useCallback(
    async (
      ...
      if (!isGeneral && screenshotList.length === 0 && !transcriptOverride && !isFollowUp && sessionMessages.length === 0) {
        setError(
    ...
    <div className="min-h-screen w-full bg-transparent text-white font-mono pointer-events-none select-none">
      ...
      {!settingsOpen && (
        <div className="flex justify-center mt-2 pb-6">
          <div className="w-[860px] space-y-2 flex flex-col h-[calc(100vh-80px)]">
            ...
            {/* Screenshots strip */}
            {screenshots.length > 0 && !currentSolution && !isStreaming && sessionMessages.length === 0 && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                      ...
                      {msg.role === "user" ? (
                        <div className="max-w-[80%] bg-white/[0.08] border border-white/[0.1] rounded-2xl rounded-tr-sm px-4 py-2 text-xs text-white/80 whitespace-pre-wrap shadow-md">
                          {msg.screenshotBase64 && (
                            <img src={msg.screenshotBase64} alt="attached context" className="max-h-24 rounded mb-2 border border-white/10" />
                          )}
                          {msg.content.includes("Here is a live interview transcript") ? "🎙️ Live Transcript Submitted" : msg.content}
                        </div>
                      ) : (
                        <div className="w-full">
                          <SolutionCard content={msg.content} isStreaming={false} />
                        </div>
                      )}
                    </div>
                  ))}
