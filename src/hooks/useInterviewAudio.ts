...
export function useInterviewAudio() {
  ...
  const whisperModel = useStore(
    (s) => s.settings.whisperModel ?? "Xenova/whisper-base.en",
  );
  ...
  // Worker Initialization — re-runs whenever the chosen Whisper model changes
  useEffect(() => {
    ...
    workerRef.current.onmessage = (e) => {
      ...
      if (type === "progress") {
        // Average progress across all shards (could be >100 momentarily, cap at 99 until ready)
        setDownloadProgress(Math.min(Math.round(e.data.progress ?? 0), 99));
      }
      if (type === "ready") {
