...
export function registerHotkeys(win: BrowserWindow): void {
  ...
  const regH = globalShortcut.register("CommandOrControl+H", async () => {
    try {
      // Briefly make window transparent so we don't capture ourselves
      // Use opacity to preserve WDA_EXCLUDEFROMCAPTURE affinity
      ...
      // Small delay to let the OS process the opacity change
